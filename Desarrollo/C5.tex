...
\textbf{Caracterización de algoritmos distribuidos}: \begin{itemize}
    \item Tipo de topología: Anillo, árbol, grafo general (dirigido o no dirigido, total o parcialmente conectado)
...
\subsection{Algoritmo de Eco-Chang}
...
\textbf{Observación 2}: Si el Eco se inicia varias veces en una misma topologia por nodos distintos, se genera lo que se denomina como \textcolor{red}{\textbf{bosque}}. Mencionar que cada iniciador tiene un árbol distinto.
\vspace{0.5em}

\textbf{Observación 3}: Aún con ciclos, el algoritmo converge.
...
\subsection{Algoritmo de instantánea distribuida}
Estos algoritmos buscan determinar el estado global del sistema sin detener la computación, típicamente se \textbf{asume un modelo sin fallas}. El fin de determinar un estado global consistente busca detectar errores, monitoreo o \textit{debugging}, gestión de recursos tales como, distribución de cargas, planificación de tareas y recolección de basura.

Dentro de los comentados existen:
\begin{itemize}
  \item Algoritmo de Chandy-Lamport
  \item Principio de la instantánea distribuida de Mattern 
\end{itemize}

\textbf{Observación:} No confundir ambos algoritmos, ambos tienen distinta notación de estados.\\
\subsubsection{Principio de la instantánea distribuida de Mattern}

\textbf{Mensajes implicados}
\begin{itemize}
  \item \textcolor{red}{\textbf{Mensajes de control:}} Estos mensajes capturan el estado local de un proceso convirtiendo un nodo de color negro a uno de color rojo.
  \item \textbf{Mensajes de base o cómputo:} Son mensajes asociados a la ejecución por defecto del sistema.\\
\end{itemize}

\textbf{Funcionamiento base}

Primero un nodo cualquiera del grafo inicia el algoritmos coloreandose de color rojo (el color rojo de un nodo se puede interpretar como que el nodo ya guardo su estado global, mientras que el color negro se interpreta como un nodo que aún no ha capturado su estado), una vez ya capturado el estado, el nodo enviará a todos sus vecinos un \textcolor{blue}{mensaje de control} (los mensajes de control colorean al nodo receptor, si el receptor ya está de color rojo, este ignorará en mensaje).

Segundo, si un nodo recibe un mensaje de control proveniente de alguno de sus vecinos, este nodo se tornará de color rojo y enviará a sus vecinos el mensaje de control correspondiente.
\\

\textbf{Mensajes del futuro}\\
Estos ocurren cuando un nodo de color rojo envía un mensaje de base a un nodo de color negro. Esto se puede manejar de dos formas, primero:
\begin{enumerate}
  \item Una si al nodo negro le llega el mensaje de base del nodo rojo, este retrasara la entrega hasta que al nodo negro le llegue un mensaje de control por algún vecino 
  \item Si al nodo le llega un mensaje de base de un nodo rojo, el nodo negro sabrá que el algoritmo inició su ejecución en alguna parte del sistema, por ende este cambiará su color a rojo.
\end{enumerate}

\textbf{Mensajes en transito}\\
Un mensaje en transito es aquel mensaje de base enviado de un nodo de color negro a un nodo o canal de color rojo. Para el manejo de estos mensajes se proponen dos formas:

\begin{enumerate}
  \item Cada proceso de color rojo memoriza los mensajes de base provenientes de un nodo negro, una vez finalizado el algoritmo, el nodo rojo actualiza su estado local con los mensajes recibidos para finalmente enviarle su estado a un proceso recolectar.
  \item Cada proceso de color rojo, una vez detecta que le llega un mensaje de base de un nodo de color negro, envía la actualización al proceso recolector de manera inmediata.
\end{enumerate}

\textbf{Observación:} Este algoritmo no considera ni tolera ejecuciones concurrentes dentro del mismo sistema, en caso de existir una ejecución concurrente, el algoritmo presenta problemas.
...
Existe la posibilidad de que se pregunte el nuevo coordinador antes de tiempo, si pasa esto pueden existir dos coordinadores \textcolor{red}{\textbf{(Cortar el algoritmo antes de tiempo)}}.
\vspace{0.5em}
...
\textbf{Claves de diseño del algoritmo}: Modelo asincrónico, con paso de mensajes unidireccional y fiable. Se elige como líder al proceso con mayor identificador. Propagar identificador máximo conocido por un nodo. Identificadores menores se extinguen y dejan de propagarse, al ser dominador por algún máximo relativo. Sólo \textbf{mayor identificador logra propagarse} y alcanzar a todos los nodos. Sólo procesos candidatos en una elección ejecutan la partida del algoritmo.
...
\textbf{Claves de diseño del algoritmo}: Similar al anterior, pero permite una elección en una topología de anillo unidireccional. Modelo asincrónico, con paso de mensajes unidireccional y fiable. Se elige como \textcolor{red}{líder al proceso con mayor identificador}. Sólo mayor identificador logra circular por todo el anillo. 

\textbf{Comentarios}: Si un iniciador le llega un mensaje de elección con su propio identificador, entonces toma conocimiento que ha sido elegido. El algoritmo se puede extender en una segunda ronda para que el ganador comunique a los otros nodos que ha sido elegido.
...
\begin{itemize}
    \item \textbf{Requisito de conectividad}: Cada proceso conoce a todos los demás y existe conectividad completa. Cualquier nodo se comunica con mediante \underline{broadcast} con todos fiablemente.
...
\textbf{Comentarios}: Se requiere una red de conexión directa entre cualquier par de nodos. Cada nodo requiere conocer a todos los demás y como se ordenan. Es necesario disponer de un cierto nivel de sincronismo para saber hasta cuánto esperar.
...
\textbf{Objetivo}:\textcolor{ForestGreen}{Garantizar que a lo más un único proceso puede ejecutar simultáneamente una sección crítica}. Se requiere evitar condiciones de carrera, como coordinar acceso a un recurso, garantizar la consistencia de los datos o mantener el correcto funcionamiento de un sistema distribuido.
...
\textbf{Caracterización}: \textcolor{red}{Requiere de un único proceso controlador que otorga el permiso para entrar a la SC}. Procesos que solicitan permiso al controlador deben esperar respuesta para ingresar a su SC. La solución requiere 3 tipos de mensajes: 
...
Con Ricart-Agrawala, el Reply (Verde en la figura) hace el trabajo de Grant (para notificar el próximo proceso que tenga acceso a la SC), eso lo hace retrasando en envío de \textcolor{green}{\textbf{reply}} hasta el final.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.2\textwidth]{img/A_Ricart_Agrawala.png}
