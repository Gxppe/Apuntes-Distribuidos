\section*{Capítulo 4: Fundamentos teóricos de la computación distribuida}
...
Definiremos la siguiente estructura para un proceso $P_i$, para n procesos.

\[\textstyle
   \left[ \mathit{Vc}[1], \mathit{Vc}[2], \dots, \mathit{Vc}[i], \dots, \mathit{Vc}[n] \right]
\]

Cada índice esta asociado a un proceso $P_i$. Sabemos que $\forall e \in E$: $Vc(e)$ representa $H(e)$.

Los procesos irán aumentando de la siguiente forma:

Para \textcolor{red}{\textbf{eventos locales}} del proceso $P_i$:

\begin{enumerate}[label=\textbf{Paso \arabic*:}, wide, labelindent=0pt]
    \item $Vc(e_i)[i] + 1$, es decir que aumentamos la marca del proceso $i$
    
    \item Asignar el valor anterior al resto de los otros procesos, al nuevo evento.
    ...
    donde los índices en verde, se mantendrán iguales para el evento local.
\end{enumerate}

Resultando en el siguiente vector para $P_i$
...
Para mensajes entre procesos solo se modifica la marca del proceso que está mandando el mensaje. Por ejemplo, si $P_i$ manda un mensaje a $P_j$, se modifica la marca del vector $P_j$ asociada a $P_i$.
...
\begin{enumerate}
 ...
 \item Condición fuerte:
  {
    ...
    {\tiny \[\textstyle
      \begin{gathered}
        ...
        e \| e' \equiv \lnot [VC(e)<VC(e')] \\
        \quad \land\, \lnot [VC(e')<VC(e)]
      \end{gathered}
    \]}
  }\end{enumerate}
 ...
 \definecolor{fifogreen}{rgb}{0.149,0.635,0.412}
 \definecolor{fifored}{rgb}{0.878,0.106,0.141}
 \begin{figure}[H]
 \centering
 \shorthandoff{>}
 \begin{circuitikz}[scale=0.5, transform shape]
 \tikzset{every node/.style={font=\small}, >=Stealth}
 ...
 % Flechas verdes ("Bien")
 \draw [color=fifogreen, very thick, ->] (1,-0.4) -- (2.3,-0.4);
 \draw [color=fifogreen, very thick, ->] (3.6,-0.4) -- (4.9,-0.4);
 \node[color=fifogreen, font=\Large] at (3,-1) {Bien};

 % Flechas rojas ("Mal")
 \draw [color=fifored, very thick, ->] (0.5,0.5) -- (5.5,0.5);
 \draw [color=fifored, very thick] (0.5,0) -- (0.5,0.5);
 \draw [color=fifored, very thick] (5.5,0.5) -- (5.5,0);
 \node[color=fifored, font=\Large] at (3,0.9) {Mal};
...
Un corte puede tener mensajes en tránsito. Denotamos la siguiente característica:

{\tiny \[
  \text{Corte valido} \neq \text{Corte consistente}
\]}

Decimos que un corte consistente contiene todos los eventos que causan a cualquier evento incluido en el corte.

\subsubsection{Algoritmo de Chandy-Lamport}
Este algoritmo sirve para definir cortes consistentes. Si el grafo tiene ciclos, un proceso puede recibir distintas marcas de distintos procesos (más adelante definiremos qué sucede en estos casos).
...
Notar que, inicialmente los nodos que están totalmente en rojo son los que por primera vez reciben la marca; por lo tanto, estos envían otra marca a sus nodos vecinos. En cambio, los nodos que solo tienen el contorno rojo y el centro blanco ya recibieron una marca y, por ende, no envían una marca.

En casos de ciclos, el algoritmo sí termina, dado que cada nodo manda exactamente una marca, independiente de la cantidad de marcas que pueda recibir.
