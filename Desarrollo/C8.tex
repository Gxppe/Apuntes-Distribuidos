...
Se usa buffering para proveer alto desempeño en acceso de datos persistentes, lo que puede considerarse como transparencia de \textcolor{ForestGreen}{persistencia}. 
...
\begin{itemize}
    ...
    \item \textbf{Modelo de almacenamiento de datos}: \textcolor{red}{Operaciones de escritura sobre el medio de almacenamiento pueden fallar}. \textcolor{Sepia}{Lecturas pueden usar un código tipo checksum para detectar errores de lectura}. El medio puede decaer perdiendose información, o puede destruirse completamente.

    \item \textbf{Procesos}: Pueden caerse ocasionalmente, perdiendo la memoria volátil. Procesos fallados se detienen (fail-stop, crash), sin mandar mensajes o escribir en disco datos erróneos. \textcolor{Fuchsia}{Existe procedimiento de recuperación basado en el uso de memoria estable}, para restaurar estado anterior a la fallas y recomenzar recuperación desde un estado consistente.

    \item \textbf{Comunicación}: \textcolor{Sepia}{Mensajes pueden sufrir retardo arbitrario, perderse, duplicarse, o corromperse}. Receptor es capaz de detectar estos errores y recuperarse.
...
\textbf{Principio de serialización}: Un \textcolor{ForestGreen}{método de control de concurrencia es correcto si es serializable}. Es decir, \textcolor{Fuchsia}{existe una secuencia equivalente en que las operaciones de cada transacción aparecen antes o después de otra transacción, pero no entremezcladas}.
...
\textbf{Operaciones en conflicto}:
...
Dos planificaciones son equivalentes si respetan el orden de precedencia para todas las operaciones en conflicto. Operaciones que no están en conflicto pueden permutarse, sin afectar el resultado de equivalencia. Es decir, \textcolor{Sepia}{una planificación serializable podría ser equivalente a diferentes planificaciones seriales}.
...
\begin{itemize}
    ...
    \item Una transacción \textcolor{blue}{no puede adquirir otro candado si ha liberado alguno}. 
\end{itemize}
...
Cada \textcolor{Fuchsia}{transacción al iniciarse recibe un $TS$ como identificador en su lugar de origen}. Cada operación (R o W) lleva el $TS$ de la transacción. \textcolor{ForestGreen}{Para cada dato $x$ se ejecuta el mayor $TS$ de lectura y de escritura}, que denotaremos respectivamente por: $RTM(x)$ y $WTM(x)$ 

\textbf{Reglas de BTO}

\begin{itemize}
    \item \textcolor{Sepia}{Regla de lectura}: Sea $TS_i$ la marca de una operación de lectura sobre $x$. Si $TS_i < WTM(x)$, se rechaza la lectura y se recomienza la transacción. Si no, se realiaz la lectura y se actualiza $RTM(x) = max(RTM(x), TS_i)$
    \item \textcolor{Sepia}{Regla de escritura}: Sea $TS_j$ la marca de operación de escritura sobre $y$. Si $TS_j < RTM(y)$ o $TS_j < WTM(y)$, se rechaza la escritura y recomienza transacción. Si no, se realiza la escritura diferida y $WTM(y) = TS_j$
\end{itemize}

El protocolo \textcolor{red}{serializa en el orden creciente de las marcas de tiempo, operaciones en conflicto son ordenadas por marca de tiempo creciente}. El protocolo \textcolor{ForestGreen}{no produce deadlock}, al no existir bloqueos y ciclos de espera las transacciones que recomienzan no dejan huella y parten con un nuevo TS (que debe ser mayor). Puede producir postergación indefinida. \textcolor{blue}{Tiene un costo significativo en memoria} para mantener información sobre TS asociado a última lectura y última escritura de cada objeto de datos. 

\subsubsection{C) Métodos optimistas}
\textbf{Principio de funcionamiento}: \textcolor{red}{Asumen que los conflictos durante la ejecución de una transacción son poco probables}, de manera de reducir el costo de estar sincronizando cada acceso a datos. Se valida al término de una transacción si existieron conflictos durante su ejecución, si no existe conflictos, se compromete; si no, se aborta. \textcolor{ForestGreen}{Serializa al menos en el orden que se van validando las transacciones.}

\textbf{Fases de un método optimista}:
\begin{itemize}
    \item \textbf{Fase de lectura}: Cuerpo de la transacción, \textcolor{Fuchsia}{copiando datos usados desde la memoria persistente (BD) a buffers en la memoria principal y volátil}. Copias que puede ser actualizada en memoria principal, pero sin respaldar a la BD
    \item \textbf{Fase de validación}: \textcolor{Sepia}{Se comprueba que no existan conflictos}.
    \item \textbf{Fase de escritura}: \textcolor{red}{Si no hay conflicto se compromete la transacción, si no es así, se aborta aplicando rollback}.  
\end{itemize}

\subsubsection{Algoritmo de Kung}
Suponiendo que T es una transacción donde $T_s$ es mayor número de secuencia al comenzar $T$ y $T_f$ el mayor número de secuencia al comienzo de cada fase de validación.

\textbf{Algoritmo}:

\begin{itemize}
    \item \textcolor{red}{Asume validez}
    \item \textcolor{Sepia}{Busca conflictos}
    \item \textcolor{blue}{Decisión: Si hubo cambios declara la transacción como inválida. Si no hubo cambios, se guardan los cambios.}
\end{itemize}

%\subsubsection{Casos de control de concurrencia}

    % \begin{itemize}
    %     \item Dropbox: Usa un \textcolor{ForestGreen}{método optimista} con un historial de versiones. Conflictos son resueltos manualmente por una transacción rechazada.
    %     \item Dynamo: Servicio de Amazon, utiliza un \textcolor{ForestGreen}{método optimista} de control de concurrencia y replicación. Se deben reconciliar versiones en conflicto.
    %     \item Google: \textcolor{Sepia}{Permite acceder a documentos concurrentemente}. Procesador de palabras ve el cursor y actualizaciones de otro usuario.
    % \end{itemize}