...
\textbf{Principio de funcionamiento}: Asumen que los conflictos durante la ejecución de una transacción, de manera de reducir el costo de estar sincronizando cada acceso a datos. Se valida al término de una transacción si existieron conflictos durante su ejecución, si no existe conflictos, se compromete; si no, se aborta. Serializa al menos en el orden que se van vvalidando las transacciones.

\textbf{Fases de un método optimista}:
\begin{itemize}
    \item \textbf{Fase de lectura}: Cuerpo de la transacción, copiando datos usados desde la memoria persistente (BD) a buffers en la memoria principal y volátil. Copias que puede ser actualizada en memoria principal, pero sin respaldar a la BD
    \item \textbf{Fase de validación}: Se comprueba que no existan conflictos.
    \item \textbf{Fase de escritura}: Si no hay conflicto se compromete la transacción, si no es así se aborta aplicando rollback.  
\end{itemize}
...
\begin{itemize}
    \item Asume validez
    \item Busca conflictos
    \item Decisión: Si hubo cambios declara la transacción como inválida. Si no hubo cambios, se guardan los cambios. 
\end{itemize}

\subsubsection{Casos de control de concurrencia}

\begin{itemize}
    \item Dropbox: Usa un método optimista con un historial de versiones. Conflictos son resueltos manualmente por una transacción rechazada.
    \item Dynamo: Servicio de Amazon, utiliza un método optimista de control de concurrencia y replicación. Se deben reconciliar versiones en conflicto.
    \item Google: Permite acceder a documentos concurrentemente. Procesador de palabras ve el cursor y actualizaciones de otro usuario.
\end{itemize}